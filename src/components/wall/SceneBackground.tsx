...
export default function SceneBackground({ theme, intensity = 'ambient', seed = 'wall', className }: Props) {
  ...
  const pieces = useMemo<Piece[]>(() => {
    ...
    const count = intensity === 'full' ? 22 : 12;
    ...
    return Array.from({ length: count }, (_, index) => {
      ...
      // Keep the middle column clear: headings and buttons live there, and
      // decoration drifting behind text is the fastest way to look cheap.
      const edge = random() < 0.5 ? random() * 26 : 74 + random() * 26;
      const top = random() * 100;
      const nearVerticalCentre = top > 26 && top < 74;

      return {
        key: `${name}-${index}`,
        name: name in DECOR_COMPONENTS ? name : 'sparkle',
        left: nearVerticalCentre ? edge : 6 + random() * 88,
        top,
        // Distant pieces are smaller and fainter — cheap but convincing depth.
        size: 12 + depth * (intensity === 'full' ? 40 : 28),
        depth,
        delay: random() * 6,
        duration: 7 + random() * 8,
        opacity: (0.1 + depth * 0.28) * (intensity === 'full' ? 1 : 0.8),
          ...
          return (
            <div
              ...
              style={{
                ...
                filter: piece.depth < 0.4 ? 'blur(2px)' : undefined,
